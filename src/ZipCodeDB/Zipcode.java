 ...
 */
public class Zipcode implements Comparable<Zipcode>, Distance, LatLngInterface {
    ...
    public WeatherObservation getWeatherData() {
        ...
        while (s.hasNextLine()) {
            ...
            if (line.indexOf("temperature") > 0) {
                temp = Double.parseDouble(line.substring(line.indexOf(':') + 3,
                        line.indexOf(',') - 1));
            }
            if (line.indexOf("windSpeed") > 0) {
                speed = Double.parseDouble(line.substring(line.indexOf(':') + 4,
                        line.indexOf(',') - 1));
            }
    ...
    public ExamDB getNorthern() {
        ...
        while (s.hasNextLine()) {
            ...
            if (line.indexOf("lat") > 0) {
                lats = Double.parseDouble(line.substring(line.indexOf(':')+1, line.indexOf(
                        ':') + 10));
            }
        }
        ExamDB obs =
